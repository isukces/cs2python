...
{
    [Serializable]
    public class Cs2PyCompiler : MarshalByRefObject, IDisposable
    {
        ~Cs2PyCompiler()
        {
            Dispose(false);
        ...
        public void LoadProject(string csProj, string configuration)
        {
            ...
            {
                var s          = parseOptions.PreprocessorSymbolNames.Union(new[] {"CS2Py"}).ToArray();
        ...
        private void EmitContentFiles(string outDir)
        {
            ...
            if (!contentFiles.Any())
                return;
            var srcDir = PathUtil.MakeWinPath(null, PathUtil.SeparatorProcessing.Append,
                PathUtil.SeparatorProcessing.Append).ToLower();
            var dstDir = PathUtil.MakeWinPath(null,
        ...
        ///     Własność jest tylko do odczytu.
        /// </summary>
        public Microsoft.CodeAnalysis.Compilation ProjectCompilation { get; private set; }
