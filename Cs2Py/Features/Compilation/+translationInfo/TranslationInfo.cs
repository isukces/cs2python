...
namespace Cs2Py.Compilation
{
    public class TranslationInfo
    {
        ...
        public PyQualifiedName GetPyType(Type type, bool doCheckAccesibility, Type relativeTo)
        {
            if ((object)type == null)
            ...
            if (type == relativeTo)
            {
                pyQualifiedName.ForceName = PyQualifiedName.ClassnameSelf;
            }
            else if (type != typeof(object) && type == relativeTo.BaseType)
            {
                pyQualifiedName.ForceName = PyQualifiedName.ClassnameParent;
            }
            else
            {
                var currentTypePy = GetPyType(relativeTo, false, null); // recurrence
                pyQualifiedName.SetEffectiveNameRelatedTo(currentTypePy);
            }
