...
namespace Cs2Py.CodeVisitors
{
    public class LangVisitor : CodeVisitor<object>
    {
        ...
        protected override object VisitSubtractAssignmentExpression(AssignmentExpressionSyntax node)
        {
            return InternalVisitExpressionAndCheckStatement(node);
        }

        protected override object VisitMultiplyAssignmentExpression(AssignmentExpressionSyntax node)
        {
            return InternalVisitExpressionAndCheckStatement(node);    
        }


        private IStatement InternalVisitExpressionAndCheckStatement(SyntaxNode node)
        {
            var result = InternalVisitExpression(node);
            if (result is IStatement statement) return statement;
            throw new Exception("Result should be IStatement");
        }

        protected override object VisitPostDecrementExpression(PostfixUnaryExpressionSyntax node)
        {
            return InternalVisitExpressionAndCheckStatement(node);
        }
        // Private Methods 
        ...
        {
            throw new Exception("Why am I here???");
            var a = InternalVisitExpression(node);
        ...
        {
            var expression = InternalVisitExpression(node.Expression);
            var sections   = new List<CsharpSwichSection>(node.Sections.Count);
            foreach (var csSection in node.Sections)
            {
                var labels = csSection.Labels.Select(a => InternalVisitExpression(a)).Cast<CsharpSwichLabel>()
                    .ToArray();
...
#warning 'Zarejestrować nazwy parametrów'
        ...
        {
            return InternalVisitExpressionAndCheckStatement(node.Expression);
        }
        ...
        protected override object VisitFieldDeclaration(FieldDeclarationSyntax node)
        {
            ...
            var type1 = InternalVisitExpression(node.Declaration.Type);
        ...
        protected override object VisitForStatement(ForStatementSyntax node)
        {
            ...
            return state.Context.DoWithLocalVariables(declaration,
                () =>
                {
                    var condition     = InternalVisitExpression(node.Condition);
        ...
        protected override object VisitLocalDeclarationStatement(LocalDeclarationStatementSyntax node)
        {
            ...
            foreach (var i in v.Declarators) context.LocalVariables.Add(new NameType(i.Name, v.Type));
...
#warning 'Zarejestrować nazwy parametrów'
        ...
        protected override object VisitParameter(ParameterSyntax node)
        {
            ...
            var _default = node.Default == null ? null : InternalVisitExpression(node.Default);
            return new FunctionDeclarationParameter(name, mod, t, _default);
        }
        ...
        {
            var v = node.Expression == null ? null : InternalVisitExpression(node.Expression);
...
//    throw new Exception("Roslyn powinien tozwiązać ten typ");
            ...
            if ((object)t.DotnetType == null)
            {
                ...
                Console.WriteLine("Brak typu dla {0} => {1}", node, t);
            }
        ...
        protected override object VisitVariableDeclarator(VariableDeclaratorSyntax node)
        {
            ...
            FieldInfo fi               = null;
            if (si is IFieldSymbol) fi = state.Context.Roslyn_ResolveField(si as IFieldSymbol);
            var name                   = _Name(node.Identifier);
            var value                  =
                InternalVisitExpression(node.Initializer); // Visit(node.Initializer) as IValue;
        ...
        private static string _Name(SyntaxToken a)
        {
            return a.ValueText;
        }

        private static IValue _OptimizeIValue(IValue n)
        {
        ...
        private FunctionDeclarationParameter[] rVisit(IEnumerable<IParameterSymbol> s)
        {
            return s.Select(i => rVisit(i)).ToArray();
        }

        private static Modifiers rVisit(RefKind refKind)
        {
            switch (refKind)
            {
                case RefKind.Out:
...
#warning 'Zarejestrować nazwy parametrów'
        ...
        private object[] Visit(IEnumerable<MemberDeclarationSyntax> i)
        {
        ...
        private static Modifiers VisitModifiers(SyntaxTokenList mod)
        {
        ...
        private readonly CompileState state;
    }
}